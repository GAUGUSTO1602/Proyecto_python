...
while fecha < fecha_final:
    if fecha == fecha1:
      print(fecha,':El saldo disponible es de $',Saldo_Inicial)
      fecha = fecha + timedelta(days=1)
    elif fecha == fecha2:
      print('Se han retirado $ 96')
      Saldo_restante = Saldo_Inicial - int(96)
      print('Saldo actual a la fecha',fecha,': $',Saldo_restante)
      fecha = fecha + timedelta(days=1)
    elif fecha == fecha3:
      print('Se ha transferido a la cuenta $ 1200')
      Saldo_restante = Saldo_restante + int(1200)
      print('Saldo actual a la fecha',fecha,': $',Saldo_restante)
      fecha = fecha + timedelta(days=1)
    elif fecha == fecha4:
      print('Se ha abonado a su cuenta un 3% de intereses')
      Intereses = float(3)
